...
leader的选举过程中kafka集群是不可用的，所有需要优化leader的选举过程，减少leader选举时间。kafka集群中有一个叫controller的特殊节点，它的职责就是管理broker的注册。如果controller检测到角色为leader的broker失效，就会从ISR中选择一个作为新的leader，这样就可以又快又省性能的方式完成多个partition的leader选举。如果controller挂了，会选举另外一个新的controller。
