...
考虑这样一个场景，从文件中读取数据并通过网络将数据传给另外一个程序。（很多服务程序都是这样的场景，包括提供静态访问资源的Web服务、FTP服务、邮件服务等等。）
...
我们来通过一个实际的案例来演示零拷贝，通过两种方式实现在客户端和服务器之前传输文件。TraditionalClient.java和TraditionalServer.java是基于传统方式实现的，使用File.read() 和Socket.send()来实现的。TraditionalServer.java是服务端代码，监听客户端连接，并且从socket中读取数据。TraditionalClient.java是客户端的代码，连接服务端，使用File.read()从文件中读取数据，使用socket.send()通过socket发送到服务端。具体代码如下：
...
类似的，TransferToServer.java和TransferToClient.java完成同样的功能，使用transferTo()方法从客户端往服务端发送文件。具体代码如下：
...
在Linux2.6内核的服务上运行两种实现方式，统计不同大小的数据发送所花费的时间。结果如表1所示：

表1 性能对比：传统实现方式和零拷贝

|文件大小|传统实现方式（ms）|零拷贝（ms）|
|:--|:--|:--|
|7MB|156|45|
|21MB|337|128|
|63MB|843|387|
...
从以上结果可以看出，transferTo()方法花费的时间比传统的方式节约了大概65%的时间。在一些有大量数据拷贝的应用中（例如Web服务），这种方式可以显著的提升性能。

### 总结

我们已经演示了transferTo()相比从一个通道读取数据然后再写进另外一个通道这种方式的优势。中间缓存拷贝，包括内核内部的缓存拷贝，都有一些性能开销。在应用中，处理大量的不同通道之间数据拷贝，零拷贝技术可以提供显著性能改善。